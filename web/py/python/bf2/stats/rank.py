...
def onPlayerConnect(player):
	...
	if player.getProfileId() > 2000:
		...
		if g_debug: print "rank.py[56]: Player %d had no profile id, can't request rank" % player.index
			
	
	if g_debug: print "rank.py[59]: Added player %d to rank checking" % (player.index)
...
def onStatsResponse(succeeded, player, stats):
	...
	if not "rnk" in stats:
		if g_debug: print "rank.py[70]: rank not found, aborting"
		return

	if g_debug: print "rank.py[73]: Rank received for player ", playerIndex, ":", host.pers_getStatsKeyVal("rnk", player.getProfileId())
	if not player: return
	

	value = int( host.pers_getStatsKeyVal("rnk", player.getProfileId()) )
	if g_debug: print "rank.py[78]: Player",player.index,"Rank:", value
