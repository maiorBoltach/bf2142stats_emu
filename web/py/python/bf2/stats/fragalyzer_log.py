...
def init():
	...
	if g_debug: print "fragalyzer_log.py[25] Fragalyzer log module initialized."
...
def enable():
	...
	if logfile and not logfile.closed:
		if g_debug: print "fragalyzer_log.py[34] Fragalyzer logging already enabled"
		return 
	...
	if g_debug: print "fragalyzer_log.py[50] log file: ", fileName

	try:
		logfile = file (fileName, 'w')
	except Exception:
		print "fragalyzer_log.py[55] Couldnt open fragalyzer logfile: ", fileName
		return
...
def disable():
	if logfile:
		...
		logfile.close()
		if g_debug: print "fragalyzer_log.py[165] Fragalyzer logging disabled."
	else:
		if g_debug: print "fragalyzer_log.py[167] Fragalyzer logging was already disabled."
